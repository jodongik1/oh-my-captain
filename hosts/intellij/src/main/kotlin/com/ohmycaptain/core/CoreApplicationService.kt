 ...
 *
 * IntelliJ 플러그인 시작 시 Node.js Core 프로세스를 구동하고,
 * 플러그인 종료 시 프로세스를 정리한다.
 *
 * ─ 역할 분담 ───────────────────────────────────────────────────
 *   IntelliJ (Kotlin) : "창구" — UI 렌더링, IDE 파일 접근, 사용자 승인 처리
 *   Core (Node.js)    : "두뇌" — LLM 호출, 도구 실행, 대화 히스토리 관리
 *   LLM 서버          : Ollama / OpenAI / Anthropic (HTTP로 연결)
 *
 ...
 *   IntelliJ(JVM)와 Core(Node.js)는 서로 다른 프로세스라 메모리를 공유할 수 없다.
 *   이 프로젝트는 stdio(표준 입출력)를 IPC 채널로 사용한다.
 *
 *   ┌─ 채널별 역할 ──────────────────────────────────────────────┐
 *   │ stdin  : Kotlin → Core  메시지 전송 (IpcClient가 JSON 씀)  │
 *   │ stdout : Core → Kotlin  메시지 전송 (Core가 JSON 씀)       │
 *   │ stderr : Core 로그 전용 → IntelliJ 로그창에 출력           │
 *   └────────────────────────────────────────────────────────────┘
 *   stdout과 stderr를 분리(redirectErrorStream=false)한 이유:
 *   stdout은 NDJSON 메시지 통신 전용이므로 로그가 섞이면 파싱 오류가 발생한다.
 *
 * @Service(APP) : IntelliJ 애플리케이션 전체에서 싱글톤으로 관리됨
 * Disposable    : 플러그인 언로드 시 dispose()가 자동 호출되어 프로세스 정리
 */
...
class CoreApplicationService : Disposable {
     ...
     *
     * @param projectRoot 현재 열린 프로젝트 루트 경로 (init 메시지로 Core에 전달됨)
    ...
    fun startCore(projectRoot: String): Process {
        ...
        Thread {
            proc.errorStream.bufferedReader().forEachLine { log.warn("[OMC] $it") }
     ...
     *       └── index.js ← 이 파일을 탐색
     *
     * @return core/index.js 절대 경로, 존재하지 않으면 null
     */
    ...
    /**
     * 플러그인 종료 또는 언로드 시 Core 프로세스를 강제 종료한다.
     * IntelliJ의 Disposable 계약에 의해 자동 호출됨.
     */
